...
object SparkImage {
  ...
  def main(args: Array[String]): Unit = {
    ...
    testImagesArray.foreach(f => {
      ...
      println("Predicting test image : " + cat + " as " + IMAGE_CATEGORIES(predictedClass.toInt))
  ...
  def extractImageDescriptors(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    if (Files.exists(Paths.get(FEATURES_PATH))) {
      println(FEATURES_PATH + " exists, skipping feature extraction..")
      return
    }
  ...
  def executeKMeansClustering(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(KMEANS_PATH))) {
      println(KMEANS_PATH + " exists, skipping clusters formation..")
      return
    }
    ...
    println("Saves Clusters to " + KMEANS_PATH)
  ...
  def createHistogram(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    if (Files.exists(Paths.get(HISTOGRAM_PATH))) {
      println(HISTOGRAM_PATH + " exists, skipping clusters formation..")
      return
    }
    ...
    val data = images.map {
      case (name, contents) => {
        ...
        if (desc != null) {
          ...
          List(categories.value.indexOf(cat) + ",[0]")
        }
      }
    }.reduce((x, y) => x ::: y)
    ...
    println ("About to save Histogram output")
  ...
  def generateRandomForestModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(RANDOM_FOREST_PATH))) {
      println(RANDOM_FOREST_PATH + " exists, skipping clusters formation..")
      return
    }
